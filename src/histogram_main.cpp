...
int main()
{
    ...
    while(1){
        ...
        if(type==None){
            ...
            /* std::unordered_map<std::string,unsigned>::iterator got = histogram.find(*yylval.wordValue);
            if(got != histogram.end()){
              unsigned tmp = histogram[*yylval.wordValue];
              histogram[*yylval.wordValue] = tmp + 1;
            }
            else{
              std::pair<std::string, unsigned> word;
              word = std::make_pair(*yylval.wordValue,1);
